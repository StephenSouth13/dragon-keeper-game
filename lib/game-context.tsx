...
const MOCK_DRAGONS: Dragon[] = [
  {
    ...
    attack: 20,
    defense: 15,
    image: "/dragon/RongAnhSang.png",
    status: "Healthy",
    description:
      "A fiery dragon, born from volcanic ash. Its breath can melt steel.",
    evolutionLevel: 10,
  ...
  },
  {
    ...
    attack: 18,
    defense: 12,
    image: "/dragon/DragonIce1.png",
    status: "Healthy",
    description:
      "A majestic ice dragon, capable of freezing entire lakes with a single roar.",
    evolutionLevel: 8,
    cooldowns: { feed: 0, train: 0, evolve: 0, skill: {} },
  ...
  },
  {
    ...
    attack: 25,
    defense: 18,
    image: "/dragon/DragonShadow.png",
    status: "Healthy",
    description:
      "A stealthy dragon of the night, its movements are as silent as shadows.",
    evolutionLevel: 12,
    cooldowns: { feed: 0, train: 0, evolve: 0, skill: {} },
...
interface GameContextType {
  ...
  startBattle: (
    playerDragonId: string,
    opponentDragonId: string
  ) => BattleState;
  performBattleAction: (
...
export const GameProvider = ({ children }: { children: ReactNode }) => {
        ...
        prevDragons.map((dragon) => {
          ...
          if (newCooldowns.skill) {
            ...
            for (const skillId in newCooldowns.skill) {
              updatedSkillCooldowns[skillId] = Math.max(
                0,
                newCooldowns.skill[skillId] - 1
              );
            }
      ...
      setDragons((prevDragons) =>
        prevDragons.map((dragon) => {
          if (dragon.id === dragonId) {
            const skin = skinId
              ? dragonSkins.find((s) => s.id === skinId)
              : undefined;
            let newAttack = dragon.attack;
            let newDefense = dragon.defense;
            ...
            toast({
              ...
              description: `${dragon.name} đã trang bị skin ${
                skin?.name || "mặc định"
              }.`,
              variant: "success", // Requires 'success' to be added to ToastProps variant type
            });
  ...
  const equipPlayerSkin = useCallback(
    (skinId: string | undefined) => {
      const skin = skinId
        ? playerSkins.find((s) => s.id === skinId)
        : undefined;
      setPlayer((prevPlayer) => ({
        ...prevPlayer,
        equippedSkinId: skinId,
      ...
      toast({
        title: "Trang bị Skin thành công!",
        description: `Bạn đã trang bị skin nhân vật ${
          skin?.name || "mặc định"
        }.`,
        variant: "success", // Requires 'success' to be added to ToastProps variant type
      });
  ...
  const startBattle = useCallback(
    (playerDragonId: string, opponentDragonId: string): BattleState => {
      ...
      return {
        ...
        turn: 0,
        log: [
          `Trận chiến giữa ${playerDragon.name} và ${opponentDragon.name} bắt đầu!`,
        ],
        isBattleOver: false,
      };
    },
  ...
  const performBattleAction = useCallback(
    (
      ...
      if (action === "attack") {
        playerDamageDealt = Math.max(
          1,
          playerDragon.attack - opponentDragon.defense / 2
        );
        opponentCurrentHp -= playerDamageDealt;
        newLog.push(
          `${playerDragon.name} tấn công ${opponentDragon.name} gây ${playerDamageDealt} sát thương.`
        );
        ...
        if (playerSkillUsed) {
          if (playerCurrentEnergy >= playerSkillUsed.energyCost) {
            ...
            newLog.push(
              `${playerDragon.name} sử dụng kỹ năng ${playerSkillUsed.name}.`
            );
            ...
            if (playerSkillUsed.type === "attack" && playerSkillUsed.damage) {
              ...
              opponentCurrentHp -= playerDamageDealt;
              newLog.push(
                `Gây ${playerDamageDealt} sát thương lên ${opponentDragon.name}.`
              );
            } else if (
              playerSkillUsed.type === "heal" &&
              playerSkillUsed.heal
            ) {
              playerHealAmount = playerSkillUsed.heal;
              playerCurrentHp = Math.min(
                playerDragon.maxHp,
                playerCurrentHp + playerHealAmount
              );
            ...
            newLog.push(
              `${playerDragon.name} không đủ năng lượng để sử dụng ${playerSkillUsed.name}.`
            );
            ...
            opponentCurrentHp -= playerDamageDealt;
            newLog.push(
              `${playerDragon.name} tấn công cơ bản gây ${playerDamageDealt} sát thương.`
            );
          }
        }
      }
      ...
      if (opponentCurrentHp <= 0) {
        ...
        setPlayer((prevPlayer) => ({
          ...prevPlayer,
          coins: prevPlayer.coins + rewardCoins,
        }));
        updatePlayerXP(rewardXp);
        setDragons((prevDragons) =>
          prevDragons.map((d) =>
            d.id === playerDragon.id
              ? { ...d, currentHp: playerCurrentHp, xp: d.xp + rewardXp / 2 }
              : d
          )
        );
        return {
      ...
      const availableOpponentSkills = opponentDragon.skills.filter(
        (s) =>
          opponentCurrentEnergy >= s.energyCost &&
          (opponentDragon.cooldowns.skill?.[s.id] || 0) === 0
      );

      if (availableOpponentSkills.length > 0 && Math.random() > 0.5) {
        // 50% chance to use skill if available
        opponentSkillUsed =
          availableOpponentSkills[
            Math.floor(Math.random() * availableOpponentSkills.length)
          ];
        opponentCurrentEnergy -= opponentSkillUsed.energyCost;
        newLog.push(
          `${opponentDragon.name} sử dụng kỹ năng ${opponentSkillUsed.name}.`
        );
        ...
        if (opponentSkillUsed.type === "attack" && opponentSkillUsed.damage) {
          ...
          newLog.push(
            `Gây ${opponentDamageDealt} sát thương lên ${playerDragon.name}.`
          );
        } else if (
          opponentSkillUsed.type === "heal" &&
          opponentSkillUsed.heal
        ) {
          opponentHealAmount = opponentSkillUsed.heal;
          opponentCurrentHp = Math.min(
            opponentDragon.maxHp,
            opponentCurrentHp + opponentHealAmount
          );
        ...
        opponentDamageDealt = Math.max(
          1,
          opponentDragon.attack - playerDragon.defense / 2
        );
        playerCurrentHp -= opponentDamageDealt;
        newLog.push(
          `${opponentDragon.name} tấn công ${playerDragon.name} gây ${opponentDamageDealt} sát thương.`
        );
      }
